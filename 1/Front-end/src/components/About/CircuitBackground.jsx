...
const CircuitBackground = ({ useWindowSize = false }) => {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      // THE KEY CHANGE: if useWindowSize is true (used in About page),
      // we always use the full window dimensions so the circuit scene
      // renders richly across the entire screen. The parent container's
      // overflow:hidden then clips it to just the hero strip height.
      // Without this, the canvas would measure its small container
      // (e.g. 380px tall) and render a tiny, empty-looking scene.
      if (useWindowSize) {
        w = window.innerWidth;
        h = window.innerHeight;
      } else {
        // Default behavior: measure the canvas element itself
    ...
    const drawBackground = () => {
      ...
      const vignette = ctx.createRadialGradient(chip.x, chip.y, 100, chip.x, chip.y, Math.max(w, h) * 0.7);
  ...
  }, [useWindowSize]); // re-runs if prop changes
